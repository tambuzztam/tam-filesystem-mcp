...
export class PromptManager {
   ...
   */
  private async discoverPrompt(name: string, searchPaths?: string[]): Promise<string> {
    const paths = searchPaths || [this.config.promptsPath, this.config.templatesPath];
  ...
  private async findExactMatch(basePath: string, name: string): Promise<string | null> {
    console.log(`findExactMatch: ${basePath}/${name} - Implementation pending`);
    return null;
  }

  private async findByAlias(basePath: string, name: string): Promise<string | null> {
    console.log(`findByAlias: ${basePath}/${name} - Implementation pending`);
    return null;
  }

  private async findFuzzyMatch(basePath: string, name: string): Promise<string | null> {
    console.log(`findFuzzyMatch: ${basePath}/${name} - Implementation pending`);
    return null;
  }

  private async findByContent(basePath: string, name: string): Promise<string | null> {
  ...
  private resolveTemplaterSyntax(content: string, variables: Record<string, any>): string {
